...
function getHeroCompositionClasses() {
  switch (siteBranding.heroComposition) {
    case "full-bleed":
      return {
        container: "relative flex min-h-[var(--industry-hero-min-height)] items-center justify-center py-20 md:py-[5.5rem] lg:py-24",
    ...
    case "centered":
      return {
        container: "grid min-h-[calc(var(--industry-hero-min-height)-4vh)] justify-center gap-8 py-20 md:py-[5.5rem] lg:py-24",
        content: "mx-auto max-w-4xl text-center",
    ...
    case "stacked":
      return {
        container: "grid gap-6 py-16 md:py-[4.5rem] lg:py-20",
        content: "max-w-4xl",
    ...
    default:
      return {
        container: "grid min-h-[var(--industry-hero-min-height)] items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] py-20 md:py-[5.5rem] lg:py-24",
        content: "",
        media: "",
        mediaFirst: false,
        contentFirst: true
    ...
    <section className="industry-hero premium-hero relative overflow-hidden border-b border-white/10 bg-shell text-white">
      ...
      <Container className={`${composition.container} ${topPaddingClass}`}>
        <div className={`${siteBranding.heroContentClassName} ${composition.content} ${composition.contentFirst ? "order-1" : "order-2"}`}>
          ...
          <p className="mt-3 max-w-4xl text-3xl font-black leading-tight text-primary md:text-5xl">{siteConfig.hero.highlight}</p>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-white/80 md:text-xl">{siteConfig.hero.subheadline}</p>
...
function HeroB({ topPaddingClass }: { topPaddingClass?: string }) {
  ...
  return (
    <section className="industry-hero premium-hero relative overflow-hidden bg-shell py-[4.5rem] md:py-20 text-white">
      ...
      <Container className={`relative ${topPaddingClass}`}>
        <div className={`industry-hero-panel rounded-[2rem] border border-white/10 bg-black/25 p-[var(--section-card-padding,var(--industry-card-padding))] backdrop-blur-xl lg:p-[calc(var(--section-card-padding,var(--industry-card-padding))+0.7rem)] ${siteBranding.heroPanelClassName} ${siteBranding.heroContentClassName} ${alignClass}`}>
          ...
          <h1 className="industry-heading mt-6 text-5xl font-black tracking-tight sm:text-6xl lg:text-7xl">{siteConfig.hero.headline}</h1>
          <p className="mt-4 text-3xl font-black text-primary">{siteConfig.hero.highlight}</p>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-200 md:text-xl md:leading-9">{siteConfig.hero.subheadline}</p>
          <HeroButtons centered={centered} />
        </div>
      </Container>
    </section>
...
function HeroC({ topPaddingClass }: { topPaddingClass?: string }) {
  ...
  return (
    <section className="industry-hero premium-hero bg-panel py-14 md:py-16">
      {siteBranding.heroComposition === "full-bleed" ? (
        <div className="industry-hero-media absolute inset-0">
      ...
      <Container className={`${composition.container} ${topPaddingClass}`}>
        <div className={`industry-hero-panel rounded-[2rem] bg-shell p-[var(--section-card-padding,var(--industry-card-padding))] text-white shadow-glow lg:p-[calc(var(--section-card-padding,var(--industry-card-padding))+0.5rem)] ${siteBranding.heroPanelClassName} ${siteBranding.heroContentClassName} ${composition.content} ${composition.contentFirst ? "order-1" : "order-2"}`}>
          ...
          <h1 className="industry-heading mt-5 text-5xl font-black tracking-tight sm:text-6xl">{siteConfig.hero.headline}</h1>
          <p className="mt-4 text-3xl font-black text-accent">{siteConfig.hero.highlight}</p>
          <p className="mt-5 text-lg leading-8 text-white/75">{siteConfig.hero.subheadline}</p>
          <HeroButtons centered={centered} />
        </div>
